...
impl SubsonicClient<'_> {
    ...
    pub async fn execute<T>(&self, endpoint: &str, method: reqwest::Method, data: Option<&str>, extra_params: Option<HashMap<String, String>>) -> Result<T, String> where T: DeserializeOwned {
        let user_agent = format!("Subsonic Client/1.0");
        ...
        default_query_params.insert("c", "OkayPlayer");
        ...
        url.query_pairs_mut().extend_pairs(extra_params.unwrap_or_default());
    ...
    pub async fn get<T>(&self, endpoint: &str, extra_params: Option<HashMap<String, String>>) -> Result<T, String> where T: DeserializeOwned {
        self.execute::<T>(endpoint, reqwest::Method::GET, None, extra_params).await
    }
}
